...
export function activate(context: vscode.ExtensionContext) {
	...
	async function updateDecorations() {
		...
		await vscode.workspace.findFiles(configGlob, '**/node_modules/**', 1).then((uris: vscode.Uri[]) => {
			if (uris && uris.length) {
				const filePath = uris[0].fsPath;
				const jsonConfigData = JSON.parse(fs.readFileSync(filePath));

				const { entity = undefined, documentation = undefined, app = undefined } = jsonConfigData || {};

				if (entity) {
					const { components = undefined, module = true, dto = true, service = true } = entity;
				...
				if (app) {
					const { routing = true, module = true, component = undefined, styles = 'css' } = app;
		...
		try {
			...
			try {
				fs.readdirSync(appFileArray.slice(1).join('/')).forEach((file: string) => {
					filesInCurrentDir.push(file);
				});
			} catch (error) {
		...
		if (filesInCurrentDir.some((file) => `${file}`.includes('.component.ts'))) {
			validRouting = filesInCurrentDir.some((file) => `${file}`.includes('app-routing.module.ts'));
			validAppModule = filesInCurrentDir.some((file) => `${file}` === 'app.module.ts');
			validAppSpec = filesInCurrentDir.some((file) => `${file}`.includes(`app.component.spec.ts`));
			validAppHtml = filesInCurrentDir.some((file) => `${file}`.includes(`app.component.html`));
			validAppStyle = filesInCurrentDir.some((file) => `${file}`.includes(`app.component.${APP_STYLING}`));
		}
		...
		vscode.workspace.findFiles(glob, '**/node_modules/**', 500).then((uris: vscode.Uri[]) => {
			if (!activeEditor) {
				return;
			}
			...
			try {
				...
				try {
					fs.readdirSync(currentDirArray.slice(1).join('/')).forEach((file: string) => {
						filesInCurrentDir.push(file);
					});
				} catch (error) {
					console.log('Catched Error reading file dir.', error);
			...
			if (filesInCurrentDir.some((file) => `${file}`.includes('.component.ts'))) {
				validSpecs = filesInCurrentDir.some((file) => `${file}`.includes('.component.spec'));
				validHTML = filesInCurrentDir.some((file) => `${file}`.includes('.component.html'));
				validStyle = filesInCurrentDir.some((file) => `${file}`.includes(`.component.${APP_STYLING}`));
			} else {
				validDTO = filesInCurrentDir.some((file) => `${file}`.includes(`${moduleName}.ts`));
				validDTOspec = filesInCurrentDir.some((file) => `${file}` === `${moduleName}.spec.ts`);

				validService = filesInCurrentDir.some((file) => `${file}`.includes(`${moduleName}.service.ts`));
				validServiceSpec = filesInCurrentDir.some((file) => `${file}`.includes(`${moduleName}.service.spec.ts`));

				validModule = filesInCurrentDir.some((file) => `${file}` === `${moduleName}.module.ts`);
			}

			// * [2] Extraer el nombre del .module
			const module: string = moduleName.charAt(0).toUpperCase() + moduleName.slice(1);
			...
			// * Verificación de DTO spec
			if (ENTITY_CLASS_SPECS && !validDTOspec) {
				hooverMessage += `\n* Component missing dto spec file: ${moduleName}.spec.ts`;
			} else if (!ENTITY_CLASS_SPECS && validDTOspec) {
				hooverMessage += `\n* Component dto spec file is not required: ${moduleName}.spec.ts`;
			}
			...
			if (ENTITY_SERVICE_SPECS && !validServiceSpec) {
				hooverMessage += `\n* Component missing service spec file: ${moduleName}.spec.ts`;
			} else if (!ENTITY_SERVICE_SPECS && validServiceSpec) {
				hooverMessage += `\n* Component service spec file is not required: ${moduleName}.spec.ts`;
			}
			...
			if (ENTITY_MODULE && VALID_ACTIONS.length && fileNameModule.toLowerCase() !== moduleName.toLowerCase()) {
			...
			if (fileFragment.length > 2) {
				fileType = fileFragment.charAt(0).toUpperCase() + fileFragment.slice(1);
			}

			if (fileCrudAction.length > 2) {
				fileActionName = fileCrudAction.charAt(0).toUpperCase() + fileCrudAction.slice(1);
			...
			if (!['ts', 'module'].includes(`${fileType}`.toLowerCase())) {
				const regEx = new RegExp(`class (?!${module}${fileActionName}${fileType})`, 'g'); // - /([A-Z][a-z])\w+/g;
				let match;
			...
			if (DOCUMENTATION_ATTRS) {
				...
				textLines.forEach((line, index) => {
					...
					if (!line.includes('@') && !line.includes('(') && !line.includes(')') && !line.includes('/') && !line.includes('*')) {
						const regExJavaDoc = new RegExp(/((?<![public|private|protected])\s[a-zA-Z0-9!#\$%\^\&*\)\(+=._-]+:[\s]*[\S]*;)/, 'g');
						let matchJavaDoc;
					...
					if (
						...
						!line.includes('this') &&
						!line.includes('if') &&
						(line.includes('private') || line.includes('public') || line.includes('protected') || line.includes('('))
					) {
						if (index > 0) {
							const prevLine: string = textLines[index - 1];

							if (!prevLine.includes('*/') && !prevLine.includes('.') && !prevLine.includes('constructor') && !prevLine.includes(',')) {
								if (activeEditor) {
